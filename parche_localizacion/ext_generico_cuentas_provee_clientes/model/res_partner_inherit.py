...
class ResPartner(models.Model):
    ...
    property_account_payable_vat_id = fields.Many2one('account.account',default=lambda self: self.env.company.property_account_payable_vat_id.id,company_dependent=True)
    property_account_receivable_vat_id = fields.Many2one('account.account',default=lambda self: self.env.company.property_account_receivable_vat_id.id,company_dependent=True)
    retention_iva_rate = fields.Float(default=lambda self: self.env.company.retention_iva_rate,company_dependent=True)
