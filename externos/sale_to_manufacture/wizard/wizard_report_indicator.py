    ...
    def averag_shrinkage(self):
        if self.report == 'AS':
            ...
            if totalR > 0 and total > 0:
                ...
                ws.write(row, 1, round(total,2), body_style)
                ws.write(row, 2, round(total_Mt2,2), body_style)
            #     # ws.write(row, 3, totalA, body_style)
                ws.write(row, 3, round(total_Mt2_t,2), body_style)
                ws.write(row, 4, round(totalR,2), body_style)
                ws.write(row, 5, '0.0', body_style)
                ws.write(row, 6, '0.0', body_style)
                ws.write(row, 7, round(total_waste_standard,2), body_style)
                ws.write(row, 8, round(total_waste,2), body_style)

                ws.write(row + 1, 1, round((total * 0.05),2), body_style)
            #     # ws.write(row + 1, 3, (totalA * 0.05), body_style)
                ws.write(row + 1, 5, round((totalR * 0.05),2), body_style)

            #     # ws.write(row + 2, 3, round((totalA / total) * 100, 2), body_style)
            #     ws.write(row + 2, 5, round((totalR / total) * 100, 2), body_style)
